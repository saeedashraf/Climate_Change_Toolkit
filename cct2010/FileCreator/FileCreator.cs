    ...
    public partial class FileCreator : Form
    {
        ...
        private void MainRun(string fileFolder)
        {
            ...
            if (pathFiles.Length > 0)
            {
                ...
                OutputRun(CruModel.Checked, pathFiles, isPcp);
            }
        }
        ...
        private void InsertToDbNewFile(string newFileName, double newLatitude, double newLongitude, double newElev, bool cru)
        {
        ...
        BEGIN
        ";
            var query =preQ+ @"insert into " + tblName + " (File_Name,Latitude,Longitude,Elev) values('" + newFileName.Replace("p.txt", "").Replace("t.txt", "") + "'," + newLatitude + "," +
                        newLongitude + "," +
                        newElev + @")
        End";
        ...
        {
            string query = "select File_name,Latitude,Longitude,Elev from cruPosition where (Latitude>=" + LatitudeFrom +
                           " and Longitude>=" + LongitudeFrom + " and  Latitude<=" + LatitudeTo +
                           " and Longitude<=" + LongitudeTo + ")";
            var dt = RunQuery(query);
