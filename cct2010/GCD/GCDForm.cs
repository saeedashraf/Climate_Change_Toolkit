...
namespace GCD
{
    public partial class GCD : Form
    {
        ...
        private void CalcFiles(string directory, string model, string senario, bool isPcp, string prefix, List<string> acceptFiles, DateTime startPeriod, DateTime endPeriod)
        {
            ...
            foreach (var file in Files)
            {
                ...
                for (int i = firstLine; i < lastLine; i++)
                {
                    ...
                    {
                        totalMonth += Convert.ToDouble(lines[i]);
                ...
                for (int i = 0; i < 13; i++)
                {
                    ...
                    {
                        columnTotal += Convert.ToDouble(dt[j, i]);
                    }
                    dt[yearCount, i] = columnTotal / yearCount;

                }
                #endregion

                var fileName = model + "_" + senario + "_" + prefix + file.Name.Replace(".txt", ".xlsx");
                SaveExcel(directory + fileName, dt, isPcp);
            }
        ...
        {
            var d = Directory.CreateDirectory(directory.Replace(@"\Average-Anomalies\Outputs_Anomaly", @"\UserDatabase"));
        ...
        private void CalcFileAnomalyCru(string directory, string model, string senario, bool isPcp, string prefix, List<string> acceptFiles, DateTime baseStartPeriod, DateTime baseEndPeriod, DateTime futStartPeriod, DateTime futEndPeriod)
        {
            ...
            var cruPath = path + @"\HistoricData\HistoricData";
            if (isPcp)
            {
                cruPath += @"\pcp";
            }
            ...
            foreach (var file in Files)
            {
                var singleOrDefault = union.SingleOrDefault(x => x.Main == file.Name.Replace("p.txt", "").Replace("t.txt", ""));
        ...
        private void SaveAnomalySummmeryExcel(List<double[]> anomalySummery, string filePath, string inputBiasFileName)
        {
            ...
            ws.SaveAs(filePath);
            ws2.SaveAs(inputBiasFolder + "Historic_" + inputBiasFileName.Replace("Anomaly", "Longterm"));
        ...
        private double[] Anomaly(FileInfo file, DateTime startPeriod, DateTime endPeriod, bool isPcp, string prefix)
        {
            ...
            for (int i = 0; i < 13; i++)
            {
                ...
                avgDt[i] = total / (yearEndIndex + 1 - yearStartIndex);
            }
            return avgDt;
        }
        private List<Position> FindFileـJoin(double LatitudeFrom, double LongitudeFrom, double LatitudeTo, double LongitudeTo)
        {
            ...
            string query = "SELECT distinct cruPosition.File_Name as cru,PositionRecord.File_Name,cruPosition.Longitude,cruPosition.Latitude from cruPosition,PositionRecord where cruPosition.Latitude=PositionRecord.Latitude and cruPosition.Longitude=PositionRecord.Longitude and (cruPosition.Latitude>=" + LatitudeFrom +
        ...
        private void SaveExcel(string filePath, double[,] data, bool isPcp)
        {
            Workbook wb = xlApp.Workbooks.Add(XlWBATemplate.xlWBATWorksheet);
            Worksheet ws = (Worksheet)wb.Worksheets[1];
            var yearCount = 94;
