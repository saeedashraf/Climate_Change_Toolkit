...
namespace GCD
{
    public partial class GCD : Form
    {
        ...
        private bool PrintIndividual = true;
        ...
        {
            var d = Directory.CreateDirectory(directory.Replace(@"\Average-Anomalies\Outputs_Average", @"\UserDatabase"));
        ...
        private void OutputRun(bool isAnomaly)
        {
            ...
            else if (ScenarioChk.CheckedItems.Contains("Historical"))
            {
                ...
                foreach (var senario in ScenarioChk.CheckedItems)
                {
                    ...
                    if (Precipitation.Checked)
                    {
                        ...
                        if (isAnomaly)
                        {
                            ...
                            {
                                CalcFileAnomaly(pcpDirectory, model.ToString(), senario.ToString(), true, "pcp_", fileList, BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                            }
                        }
                        else
                        {
                    ...
                    if (Temperature.Checked)
                    {
                        ...
                        if (isAnomaly)
                        {
                            ...
                            {
                                CalcFileAnomaly(tmpDirectory, model.ToString(), senario.ToString(), false, "max_", fileList,
                               BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                                CalcFileAnomaly(tmpDirectory, model.ToString(), senario.ToString(), false, "min_", fileList,
                                    BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
        ...
        {
            var d = Directory.CreateDirectory(directory.Replace(@"\Average-Anomalies\Outputs_Anomaly", @"\UserDatabase").Replace(@"\Average-Anomalies\Outputs_Average_Anomaly", @"\UserDatabase"));
        ...
        private void CalcFileAnomalyCru(string directory, string model, string senario, bool isPcp, string prefix, List<string> acceptFiles, DateTime baseStartPeriod, DateTime baseEndPeriod, DateTime futStartPeriod, DateTime futEndPeriod)
        {
            ...
            var d = Directory.CreateDirectory(directory.Replace(@"\Average-Anomalies\Outputs_Average_Anomaly", @"\UserDatabase"));
        ...
        {
            MessageBox.Show(@"Always browse the location of UserDatabase in the project", "Tip", MessageBoxButtons.OK);
        ...
        private void RunAnomaly_Click(object sender, EventArgs e)
        {
            ...
            {
                path = FileTxt.Text;
                PrintIndividual = PrintIndividualChk.Checked;
                var inputs = "";
                var models = ModelChk.CheckedItems.Cast<object>().Aggregate("", (current, model) => current + (model + ","));
        ...
        {
            ScenarioChk.SetItemChecked(0, true);
            ScenarioChk.Enabled = false;
        ...
        {
            path = FileTxt.Text;
            var mainInput = path.Replace("UserDatabase", "inputs.txt");
