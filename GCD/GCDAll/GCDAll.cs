...
namespace GCDAll
{
    public partial class GCDAll : Form
    {
        ...
        private void OutputRun(bool isAnomaly)
        {
            ...
            var FutCheckedModels = FutModelChk.CheckedItems.OfType<string>().ToList();
            var FutCheckedSens = FutScenarioChk.CheckedItems.OfType<string>().ToList();

            List<string> fileList = null;


            startDate = new DateTime(2006, 01, 01);
            futStartDate = new DateTime(2006, 01, 01);
            var startPeriod = new DateTime(2006, 1, 1);
            var endPeriod = new DateTime(2099, 12, 31);

            if (CheckedSens.Contains("Historical"))
            {
                startDate = new DateTime(1950, 01, 01);
                startPeriod = new DateTime(1950, 1, 1);
                endPeriod = new DateTime(2006, 12, 31);
            }
            if (FutCheckedSens.Contains("Historical"))
            {
                futStartDate = new DateTime(1950, 01, 01);
            }
            ...
            foreach (var model in FutCheckedModels)
            {
                var modelDirectory = outputPath + @"\" + model;
                Directory.CreateDirectory(modelDirectory);
                foreach (var senario in FutCheckedSens)
                {
                    ...
                    if (Precipitation.Checked)
                    {
                        ...
                        if (baseCru.Checked)
                        {
                            CalcFileAnomalyCru(pcpDirectory, model.ToString(), senario.ToString(), true, "pcp_", fileList,
                                BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                        }
                        else if (baseObserve.Checked)
                        {
                            CalcFileAnomalyObserve(pcpDirectory, model.ToString(), senario.ToString(), true, "pcp_", fileList,
                               BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);

                        }
                        else
                        {
                            foreach (var baseModel in CheckedModels)
                            {
                                var BmodelDirectory = outputPath + @"\" + baseModel;
                                Directory.CreateDirectory(BmodelDirectory);
                                foreach (var baseSenario in CheckedSens)
                                {
                                    var BsenarioDirectory = BmodelDirectory + @"\" + baseSenario;
                                    Directory.CreateDirectory(senarioDirectory);
                                  
                                        var basePcpDirectory = BsenarioDirectory + @"\pcp\";
                                        CalcFileAnomaly(pcpDirectory, basePcpDirectory,baseModel.ToString(),baseSenario.ToString(), model.ToString(), senario.ToString(), true, "pcp_", fileList, BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value );
                    ...
                    if (Temperature.Checked)
                    {
                        ...
                        if (baseCru.Checked)
                        {
                            CalcFileAnomalyCru(tmpDirectory, model.ToString(), senario.ToString(), false, "max_", fileList,
                           BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                            CalcFileAnomalyCru(tmpDirectory, model.ToString(), senario.ToString(), false, "min_", fileList,
                                BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);

                        }
                        else if (baseObserve.Checked)
                        {
                            CalcFileAnomalyObserve(tmpDirectory, model.ToString(), senario.ToString(), false, "max_", fileList,
                        BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                            CalcFileAnomalyObserve(tmpDirectory, model.ToString(), senario.ToString(), false, "min_", fileList,
                                BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                        }
                        else
                        {
                            ...
                            foreach (var baseModel in CheckedModels)
                            {
                                ...
                                foreach (var baseSenario in CheckedSens)
                                {
                                    ...
                                    Directory.CreateDirectory(senarioDirectory);
                                        ...
                                        var basetmpDirectory = BsenarioDirectory + "/tmp/";
                                       
                                    CalcFileAnomaly(tmpDirectory, basetmpDirectory, baseModel.ToString(), baseSenario.ToString(), model.ToString(), senario.ToString(), false, "max_", fileList,
                          BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
                                    CalcFileAnomaly(tmpDirectory, basetmpDirectory, baseModel.ToString(), baseSenario.ToString(), model.ToString(), senario.ToString(), false, "min_", fileList,
                                        BaseStartDate.Value, BaseEndDate.Value, FutStartDate.Value, FutEndDate.Value);
        ...
        private void CalcFileAnomaly(string directory, string baseDirectory, string baseModel, string baseSenario, string model, string senario, bool isPcp, string prefix, List<string> acceptFiles, DateTime baseStartPeriod, DateTime baseEndPeriod, DateTime futStartPeriod, DateTime futEndPeriod)
        {
            ...
            var baseD = Directory.CreateDirectory(baseDirectory.Replace(@"\Average-Anomalies\Outputs_Anomaly", UserDataFolder).Replace(@"\Average-Anomalies\Outputs_Average_Anomaly", UserDataFolder));
            ...
            List<Position> union = new List<Position>();
          
            if (ReadFromObservedDb)
            {
                union = FindFileNamesObserve();
            }
            else
            {
                union = FindFileـJoin(LatitudeFrom, LongitudeFrom, LatitudeTo, LongitudeTo);
            }

            var Files = d.GetFiles();
            var BaseFiles = baseD.GetFiles();
            if (acceptFiles != null)
            {
                Files = Files.Where(x => acceptFiles.Any(y => y == x.Name.Replace("p.txt", "").Replace("t.txt", ""))).ToArray();
            }
            ...
            foreach (var file in Files)
            {
                ...
                var baseFile = BaseFiles.Where(x => x.Name == file.Name).SingleOrDefault();
                var baseDt = Anomaly(baseFile,startDate, baseStartPeriod, baseEndPeriod, isPcp, prefix);
                var futDt = Anomaly(file,futStartDate, futStartPeriod, futEndPeriod, isPcp, prefix);



                var fileName = "Anomaly_" + baseModel + "_" + baseSenario + "_" + model + "_" + senario + "_" + prefix + file.Name.Replace(".txt", ".xlsx");
            ...
            var totalFileName = "LongtermAve_Anomaly_" + baseModel + "_" + baseSenario + "_" + model + "_" + senario + "_" + prefix + "TotalReport.xlsx";
            var inputBiasFileName = "Longterm_" + baseModel + "_" + baseSenario + "_" + model + "_" + senario + "_" + prefix + "TotalReport.xlsx";
        ...
        private void CalcFileAnomalyCru(string directory, string model, string senario, bool isPcp, string prefix, List<string> acceptFiles, DateTime baseStartPeriod, DateTime baseEndPeriod, DateTime futStartPeriod, DateTime futEndPeriod)
        {
            ...
            foreach (var file in Files)
            {
                ...
                singleOrDefault = union.SingleOrDefault(x => x.Main == file.Name.Replace("p.txt", "").Replace("t.txt", ""));
                if (singleOrDefault != null && singleOrDefault.Main.Length > 0)
                {
                    var cruName = singleOrDefault.Main;
                    var cruFile = CruFiles.SingleOrDefault(x => x.Name.Replace("p.txt", "").Replace("t.txt", "") == cruName);
                    if (cruFile != null)
                    {
                        startDate = new DateTime(1970, 01, 01);
                        var baseDt = Anomaly(cruFile, startDate, baseStartPeriod, baseEndPeriod, isPcp, prefix);
                        ...
                        var futDt = Anomaly(file, startDate, futStartPeriod, futEndPeriod, isPcp, prefix);
            ...
            foreach (var file in CruFiles)
            {
                ...
                if (singleOrDefault != null)
                {
                    ...
                    if (MainFile != null)
                    {
                        ...
                        var baseDt = Anomaly(file, startDate, baseStartPeriod, baseEndPeriod, isPcp, prefix);
                        ...
                        var futDt = Anomaly(MainFile, startDate, futStartPeriod, futEndPeriod, isPcp, prefix);
        ...
        private double[] Anomaly(FileInfo file, DateTime startFileDate, DateTime startPeriod, DateTime endPeriod, bool isPcp, string prefix)
        {
            double[,] dt = new double[95, 13];
            var lines = File.ReadAllLines(file.FullName);
            var firstLine = (startPeriod - startFileDate).Days + 1;
            var lastLine = (endPeriod - startFileDate).Days + 1;
            var mounthCounter = startPeriod.Month;
            var yearIndex = startPeriod.Year - startFileDate.Year;
            var curDate = startPeriod;
            double totalMonth = 0;
            double totalYear = 0;
            lastLine = lastLine + 2 > lines.Length ? lines.Length : lastLine + 2;
            ...
            var yearStartIndex = startPeriod.Year - startFileDate.Year;
            var yearEndIndex = endPeriod.Year - startFileDate.Year;
        ...
        private List<Position> FindFileـJoinObserve(bool cru = false)
        {
            ...
            if (ScenarioChk.CheckedItems.Contains("Historical") || cru)
            {
        ...
        {
            ReadFromObservedDb = radioButton2.Checked;
        }

        private void baseCru_CheckedChanged(object sender, EventArgs e)
        {
            baseObserve.Enabled = !baseCru.Checked;
        }

        private void baseObserve_CheckedChanged_1(object sender, EventArgs e)
        {
            baseCru.Enabled = !baseObserve.Checked;
        }
    }
}
